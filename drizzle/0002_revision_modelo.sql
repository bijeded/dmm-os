...
CREATE TABLE `__new_ubicaciones_archivo` (
...
INSERT INTO `__new_ubicaciones_archivo`("id", "proyecto_id", "tipo", "ruta_relativa", "disponible", "verificado_en") SELECT "id", "proyecto_id", "tipo", "ruta_relativa", "disponible", "verificado_en" FROM `ubicaciones_archivo`;--> statement-breakpoint
DROP TABLE `ubicaciones_archivo`;--> statement-breakpoint
ALTER TABLE `__new_ubicaciones_archivo` RENAME TO `ubicaciones_archivo`;--> statement-breakpoint
...
CREATE TABLE `__new_definiciones_ingreso` (
	...
	FOREIGN KEY (`proyecto_id`) REFERENCES `proyectos`(`id`) ON UPDATE no action ON DELETE restrict,
	FOREIGN KEY (`contacto_id`) REFERENCES `contactos`(`id`) ON UPDATE no action ON DELETE restrict,
	CONSTRAINT "definiciones_ingreso_total" CHECK("__new_definiciones_ingreso"."total" = "__new_definiciones_ingreso"."subtotal" + "__new_definiciones_ingreso"."iva"),
...
--> statement-breakpoint
INSERT INTO `__new_definiciones_ingreso`("id", "cotizacion_id", "proyecto_id", "contacto_id", "tipo", "categoria", "subtotal", "iva", "total", "monto_original", "moneda_original", "dia_del_mes", "periodo_inicio", "periodo_fin", "numero_parcialidades", "creado_en") SELECT "id", "cotizacion_id", "proyecto_id", "contacto_id", "tipo", "categoria", "subtotal", "iva", "total", "monto_original", "moneda_original", "dia_del_mes", "periodo_inicio", "periodo_fin", "numero_parcialidades", "creado_en" FROM `definiciones_ingreso`;--> statement-breakpoint
...
CREATE TABLE `__new_vigencias_precio` (
	...
	CONSTRAINT "vigencias_total" CHECK("__new_vigencias_precio"."total" = "__new_vigencias_precio"."subtotal" + "__new_vigencias_precio"."iva")
);
--> statement-breakpoint
INSERT INTO `__new_vigencias_precio`("id", "definicion_costo_id", "desde", "subtotal", "iva", "total", "monto_original", "moneda_original") SELECT "id", "definicion_costo_id", "desde", "subtotal", "iva", "total", "monto_original", "moneda_original" FROM `vigencias_precio`;--> statement-breakpoint

...
Something the importer guessed that waits in Logs to be answered once: by accepting the guess, rejecting it, or choosing another of its opciones. Three kinds: *vincular* a record to a Proyecto (e.g. CFDI → Proyecto by amount/date, or a folder taken to deliver a quote of the same name), *fusionar* two Contactos whose names are near-duplicates, and *ubicación* — asking whether a completed Proyecto with no folder is Archivado or No disponible. Its opciones are the other Proyectos of the record's Contacto for *vincular* (for a Cotización, only those without a Cotización), or the other Contactos to merge into for *fusionar*; *ubicación* has none. Answering with another opción records the Sugerencia as *corregida*. A guessed status is written so the record is usable; the suggestion is what makes it reversible.

**Importación**:
A run that brings existing files into the app: the folder scan (Cotizaciones PDFs, Clientes and Proyectos folders) or the Facturas run (issued CFDI XML). What it creates is marked as imported, which is what Reimportar desde cero removes. Each file imports in one transaction, and a Sugerencia de importación is asked only once. A rejected Sugerencia undoes exactly what its guess changed; a corrected one undoes the same, then links or merges the chosen opción.

...
Something the importer guessed that waits in Logs to be answered once: by accepting the guess, rejecting it, or choosing another of its opciones. Four kinds: *vincular* a record to a Proyecto (e.g. CFDI → Proyecto by amount/date, or a folder taken to deliver a quote of the same name), *fusionar* two Contactos whose names are near-duplicates, *ubicación* — asking whether a completed Proyecto with no folder is Archivado or No disponible — and *partidas*, shown as "¿Qué aceptó?", asking which prices of a legacy Cotización accepted by import the Contacto took. Its opciones are the other Proyectos of the record's Contacto for *vincular* (for a Cotización, only those without a Cotización), the other Contactos to merge into for *fusionar*, and the Cotización's prices for *partidas*, of which several may be chosen; *ubicación* has none. A *partidas* is pre-checked with the prices an issued invoice to the Contacto adds up to, sets the Cotización's Monto to the prices chosen, and is asked only while its Cotización is aceptada. Answering with another opción records the Sugerencia as *corregida*. A guessed status is written so the record is usable; the suggestion is what makes it reversible.

**Importación**:
A run that brings existing files into the app: the folder scan (Cotizaciones PDFs, Clientes and Proyectos folders) or the Facturas run (issued CFDI XML). What it creates is marked as imported, which is what Reimportar desde cero removes. The folder scan reads a legacy Cotización's PDF the first time its Folio is imported: fecha (in the year of its folder), items, Monto, categoría, the project it names in curly quotes, and currency. Every quote price is before IVA, so it imports with IVA 0 and a total equal to its Monto; a quote with only recurring prices is billed mensual. Each file imports in one transaction, and a Sugerencia de importación is asked only once. A rejected Sugerencia undoes exactly what its guess changed; a corrected one undoes the same, then links or merges the chosen opción.
...
A `Proyectos/` folder (in `Proyectos/`, `Archivo/Proyectos/` or on the external HDD) is attributed, in order, to its Mapa de nombres row, to an existing Contacto of the folder's name, or to the one Contacto whose Cotizaciones deliver a Proyecto of that name (the project a PDF names in curly quotes, a map row's `proyecto`, or a `Cliente - Proyecto` split), with that name as its Cliente final unless the map gives one. A project folder never creates a Contacto the map does not name; otherwise it is a **Proyecto sin Contacto**. A Cotización linked by import gives its Proyecto its categoría (over `other`) and its fecha as a missing fecha de inicio; imported Proyectos get no fecha de fin or entrega.

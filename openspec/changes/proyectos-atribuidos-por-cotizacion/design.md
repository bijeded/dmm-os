...
- **Split `atribuir` by origin instead of adding flags.** `importarCarpetaProyecto` calls a new `atribuirCarpetaProyecto(tx, mapa, nombre)` returning `{ contactoId: number | null, proyecto, clienteFinal, ambiguos: string[], ...Aportes }`:
  ...
  4. The Contactos of every Cotización for which `repartirNombres(c, nombre).nombre !== undefined`. Exactly one → that Contacto, and `clienteFinal = clienteFinalDeCotizacion(mapa, c) ?? nombre`, where `c` is the Cotización `vincularCotizacion` will link: the oldest unlinked one, or the oldest when all are linked. The lookup selects only the columns it needs, not the items JSON.
...
- **An already imported folder is found by its recorded location, not by attribution.** `importadoSinMapa` splits into `contactoImportado` (for `Clientes/`) and `proyectoImportado`. `proyectoImportado` first looks for a Proyecto whose top-level folder of the same name (by `clave`) is recorded in another root, whatever its Contacto is now, so a Contacto assigned by hand or a map row added since cannot split one folder into two Proyectos. It then falls back to the pre-change rule: a Proyecto of the folder's name under a Contacto of that name, which also covers a Proyecto created for an accepted Cotización with no folder. When nothing was imported, a Proyecto sin Contacto is matched by name with `isNull(proyectos.contactoId)` and `etiqueta = 'cliente'`, so a personal Proyecto is never reused. `vincularCotizacion` is skipped when `contactoId` is null. `anotarConocido` takes a null Contacto, so a map row for an already imported Proyecto sin Contacto counts as found rather than *sin uso*.
...
- [An Ingreso entered on a Proyecto sin Contacto takes no Contacto, as an Ingreso takes its Proyecto's, and keeps none after the Proyecto is assigned one] → The Facturas run never guesses a Proyecto sin Contacto, so only a hand-entered Ingreso can hit this. Assign the Contacto first.
- [When Cotizaciones of several Contactos name a folder and one of them is already *aceptada*, the pass for accepted quotes with no folder also creates a completed Proyecto of that name under its Contacto] → This already happened before this change, when the folder minted its own Contacto. On a first import every legacy quote is *enviada*, so it only arises for quotes accepted in the app. A map row settles it.

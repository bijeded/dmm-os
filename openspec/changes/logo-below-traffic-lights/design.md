...
- Every page title starts 72px from the window top (48px header + 24px padding). All pages use `tituloCls` (`estilos.ts`) except `SectionPage` and `Configuracion`, which repeat the same classes. Where the title shares a row with buttons (`items-center`), the 35.4px title line is the tallest item, so the title's position doesn't change.
